...
def run_cron():
    ...
    # ── LLM Filtre + Trade Yönetimi ──────────────────────────────
    ANTHROPIC_KEY = os.environ.get("GROQ_API_KEY", "")
    
    # Mevcut açık trade'leri Supabase'den çek
    try:
        req = urllib.request.Request(
            f"{SUPABASE_URL}/rest/v1/trades?status=eq.OPEN&order=created_at.desc",
            headers={**headers, "Range": "0-9"},
            method="GET"
        )
        with urllib.request.urlopen(req) as r:
            open_trades = _json.loads(r.read())
    except Exception as e:
        print(f"[CRON] Trade çekme hatası: {e}")
        open_trades = []

    print(f"[CRON] Açık trade: {len(open_trades)}")

    # Açık trade'leri yönet (TP/SL/Rejim kontrolü)
    ...
    for t in open_trades:
        ...
        if t["dir"] == "LONG":
            if spot <= t["stop"]:
                ...
                close_reason = "REJIM"
                exit_price = spot
            elif spot >= t["tp"] and not t.get("partial_closed"):
                close_reason = "TP"
                exit_price = t["tp"]
        elif t["dir"] == "SHORT":
            if spot >= t["stop"]:
        ...
        if close_reason and exit_price:
            ...
            update = {
                ...
                "exit_date": _dt2.datetime.utcnow().isoformat(),
                "pnl": round(pnl, 2),
                "notes": (t.get("notes") or "") + f" | {close_reason}"
            }
            try:
                req = urllib.request.Request(
                    f"{SUPABASE_URL}/rest/v1/trades?id=eq.{t['id']}",
                    data=_json.dumps(update).encode(),
                    headers={**headers, "Prefer": "return=minimal"},
                    method="PATCH"
                )
                urllib.request.urlopen(req)
                print(f"[CRON] Trade kapatıldı: {t['dir']} {close_reason} PnL:{round(pnl,2)}")
            except Exception as e:
                print(f"[CRON] Trade kapatma hatası: {e}")

    # Yeni trade aç — LLM filtre ile
    if len(open_trades) == 0 and ANTHROPIC_KEY:
        ga = data.get("gamma_analysis", {})
        flip_near = ga.get("flip_near", False)
        expiry_day = data.get("expiry", {}).get("expiry_day", False)

        if not flip_near and not expiry_day and (bull or bear):
            # FOMC metnini çek
            fomc = fetch_fomc_statement()
            mq = data.get("menthorq", {})
            gamma_score = mq.get("score", 0)
            gamma_regime = data.get("gamma_regime", "NEUTRAL")
            action = "LONG" if bull else "SHORT"

            deribit_summary = (
                f"P/C OI: {data.get('pc_ratio','?')} | "
                f"Front IV: {data.get('front_iv','?')}% | "
                f"IV Rank: {data.get('iv_rank','?')}% | "
                f"Net GEX: {data.get('total_net_gex','?')}M | "
                f"Term Shape: {data.get('term_shape','?')}"
            )
            onchain_summary = (
                f"Flip mesafesi: {ga.get('flip_distance_pct','?')}% | "
                f"Max Pain: {data.get('max_pain','?')} | "
                f"Expiry: {data.get('expiry',{}).get('days_to_expiry','?')} gün"
            )

            prompt = f"""Sen bir BTC options trading risk filtresinsin.
Gamma sistemi {action} sinyali üretti (skor: {gamma_score:+.3f}, rejim: {gamma_regime}).
FED/FOMC [{fomc.get('date','')}]: {fomc.get('text','')[:800]}
DERİBİT: {deribit_summary}
GEX/ON-CHAIN: {onchain_summary}
Sadece JSON döndür: {{"verdict":"ONAYLA"|"VETO"|"NÖTR","confidence":<0-1>,"reasoning":<50 kelime Türkçe>,"veto_reasons":null}}"""
            ...
            try:
                ...
                with urllib.request.urlopen(req, timeout=30) as r:
                    ...
                    if match:
                        llm_result = _json.loads(match.group(0))
                        llm_verdict = llm_result.get("verdict", "VETO")
                        print(f"[CRON] LLM: {llm_verdict} ({llm_result.get('reasoning','')})")
            except Exception as e:
                print(f"[CRON] LLM hatası: {e}")

            if llm_verdict == "ONAYLA":
                fs = data.get("layer_budget", {}).get("final_scalar", 1.0)
                risk = 10000 * 0.02 * 2 * fs
                if action == "LONG":
                    entry = spot
                    stop = max(data.get("put_support", spot * 0.95), spot * 0.95)
                    tp = data.get("call_resistance", spot * 1.05)
                else:
                    entry = spot
                    stop = data.get("call_resistance", spot * 1.05)
                    tp = data.get("put_support", spot * 0.95)
                ...
                trade = {
                    ...
                    "regime": regime,
                    "status": "OPEN",
                    "notes": f"Auto {action} GEX:{gex}M scalar:{fs} [LLM:ONAYLA]",
                    "created_at": _dt3.datetime.utcnow().isoformat()
                }
                try:
                    req = urllib.request.Request(
                        f"{SUPABASE_URL}/rest/v1/trades",
                        data=_json.dumps(trade).encode(),
                        headers={**headers, "Prefer": "return=representation"},
                        method="POST"
                    )
                    urllib.request.urlopen(req)
                    print(f"[CRON] Trade açıldı: {action} @{entry} stop:{stop} tp:{tp} [LLM ONAYLA]")
                except Exception as e:
                    print(f"[CRON] Trade açma hatası: {e}")
            else:
                print(f"[CRON] Trade açılmadı: LLM {llm_verdict}")
    elif not ANTHROPIC_KEY:
        print("[CRON] ANTHROPIC_API_KEY yok - LLM filtre atlandı")
