#!/usr/bin/env python3
"""
backtest_trader_v2.py

GERÇEK gdive_trader.py mantığıyla birebir uyumlu backtest.
v1 (backtest_trader_technical.py) DTE_EXIT'i "trade süresi >= X" diye yanlış uyguluyordu —
gerçek trader commit 0cd1723'ten beri TIME_EXIT kullanıyor.

v2 değişiklikleri:
  1. DTE_EXIT (her zaman tetiklenen) → TIME_EXIT (trade süresi >= 7g VE sadece karda)
  2. REGIME_EXIT eklendi: SHORT için bull_tech AND NOT bear_tech AND karda → kapat
                          LONG için bear_tech AND NOT bull_tech AND karda → kapat
  3. dte_exit_bars → dte_exit (gün cinsinden)

Hâlâ kapsam dışı (tarihsel veri yok):
  - GEX > 0 / GEX < 0 filtresi
  - HVL filtresi
  - Flip-zone DANGER/CAUTION
  - Pyramid totalScore
  - Taleb signal

Yani backtest hâlâ "saf teknik + gerçek exit mantığı" simülasyonu.
Live trader bu teknikere ek olarak GEX+HVL filtresi de istiyor.

Çalıştır:
  python3 backtest_trader_v2.py
"""
...
STRATEGIES = {
    "C1": {
        ...
        "dte_exit": 14,  # gün — TIME_EXIT için (eskiden dte_exit_bars=14*6)
        "trend_confirm_e200": True,
        "base_risk": 0.02,
        "leverage": 2,
        "description": "C1 Conservative",
    },
    "C4": {
        "name": "C4_Aggressive",
        "atr_stop_mult": 1.5,
        ...
        "dte_exit": 7,   # gün — TIME_EXIT için
        "trend_confirm_e200": True,
        "base_risk": 0.02,
        "leverage": 2,
        "description": "C4 Aggressive (canlı sistem)",
    },
}
...
# ──────────────────────────────────────────────────────────────
# BACKTEST v2 — TIME_EXIT + REGIME_EXIT mantığıyla
# ──────────────────────────────────────────────────────────────
def backtest(candles, cfg):
    ...
    for i in range(start_i, len(candles)):
        ...
        # Her bar'da bull_tech/bear_tech hesapla (REGIME_EXIT için gerek)
        ...
        if in_trade:
            ...
            days_held = bars_held / 6.0  # 4H bar → 6 bar/gün
            ...
            if direction == "LONG":
                ...
                # 2) TP
                    ...
                    # 3) TIME_EXIT — gerçek trader: trade_days_held >= cfg["dte_exit"] AND pnl > 0
                    if days_held >= cfg["dte_exit"]:
                        pnl_check = calc_realistic_pnl(entry, price, size, "LONG", days_held, cfg["leverage"])
                        if pnl_check["net"] > 0:
                            exit_price = price
                            exit_reason = "TIME_EXIT"
                    # 4) REGIME_EXIT — bear_tech AND NOT bull_tech AND pnl > 0
                ...
                # 2) TP
                    ...
                    if days_held >= cfg["dte_exit"]:
                        ...
                        if pnl_check["net"] > 0:
                            exit_price = price
                            exit_reason = "TIME_EXIT"
                    # 4) REGIME_EXIT — bull_tech AND NOT bear_tech AND pnl > 0
...
def analyze(trades, label):
    ...
    print(f"  Ortalama tutma:      {avg_hold:.1f} gün")
    print(f"  Avg kazanç:          ${avg_win:+.2f}")
    print(f"  Avg kayıp:           ${avg_loss:+.2f}")
    print(f"  Avg K/Z oranı:       {abs(avg_win/avg_loss) if avg_loss else 0:.2f}× (kazanç/kayıp)")
    ...
    print(f"  CAGR (yıllık):       {cagr:+.1f}%")
    print(f"  Max DD:              {max_dd_pct:.1f}%")
    print(f"  Sharpe (yıllık):     {sharpe_annual:.2f}")
    print()
    ...
    for r in sorted(exit_reasons.keys()):
        ...
        print(f"    {r:12}: {len(pnls):>3} trade  ({wins_r}W/{len(pnls)-wins_r}L)  avg ${avg:+.2f}  sum ${sum(pnls):+.2f}")
...
def main():
    ...
    if len(candles) < 250:
        print(f"⚠ Veri çok az ({len(candles)})")
        return

    print(f"\n{'='*70}")
    print(f"BACKTEST v2 — TIME_EXIT + REGIME_EXIT (gerçek trader birebir)")
    print(f"GEX/HVL/Flip-zone filtreleri DAHİL DEĞİL (tarihsel veri yok)")
    print(f"{'='*70}")

    all_results = {}
    for code, cfg in STRATEGIES.items():
        trades = backtest(candles, cfg)
        analyze(trades, f"{code} — {cfg['name']}")
        all_results[code] = trades

    print(f"\n{'='*70}")
    print(f"KARŞILAŞTIRMA TABLOSU")
    print(f"{'='*70}")
    print(f"{'Strateji':<22} {'Trade':>6} {'WR%':>6} {'Hold':>5} {'Net P&L':>10} {'CAGR%':>8} {'DD%':>7} {'Sharpe':>7}")
    for code, trades in all_results.items():
        if not trades:
            print(f"{code:<22} {'(no trades)'}")
            continue
        n = len(trades)
        ...
        print(f"{name:<22} {n:>6} {wr:>6.1f} {avg_hold:>5.1f} ${net:>+9.2f} {cagr:>+7.1f} {max_dd:>6.1f} {sharpe:>7.2f}")
