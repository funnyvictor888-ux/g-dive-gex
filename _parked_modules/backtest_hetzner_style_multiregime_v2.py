...
def fetch_with_retry(url, max_retries=3):
    """Deribit'e retry mantığıyla çağrı."""
    for attempt in range(max_retries):
        try:
            ...
            if attempt < max_retries - 1:
                print(f"    ⚠ {type(e).__name__}, {wait}s sonra tekrar dene ({attempt+1}/{max_retries})")
                time.sleep(wait)
            else:
...
def fetch_period_daily(start_iso, end_iso):
    ...
    # Daily: pagination YOK. Deribit bir yili tek cagri da verir (1D, ~365 bar).
    # Eski bug: cursor = ticks[-1]+1 (1 ms) -> ayni pencereyi sonsuz tekrar cekiyordu.
    all_ticks = []
    all_closes = []
    ...
    if data is not None:
        ...
        all_ticks = result.get("ticks", [])
        all_closes = result.get("close", [])
...
def main():
    ...
    for label, start, end, regime in PERIODS:
        ...
        # Her dönem arasında 3s nice — Deribit rate limit'i koru
        time.sleep(3.0)
        if len(bars) < 60:
